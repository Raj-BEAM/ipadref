...
-export([game_loop/1, guess/2, new_game/2, new_user/1,
	 server_loop/0, start_server/0, stop_server/0]).

start_server() ->
    register(game_server, spawn(?MODULE, server_loop, [])).

stop_server() ->
    game_server ! {self(), stop}, get_response().
...
guess(User_name, Guessed_number) ->
    game_server !
      {self(), guess, User_name, Guessed_number},
    get_response().

get_response() ->
    receive Response -> Response after 1000 -> ok end.
...
server_loop(User_db) ->
    receive
	  ...
	  case maps:find(User_name, User_db) of
	    error ->
		From ! {error, "User not found."}, server_loop(User_db);
	    {ok, _X} ->
      ...
      {From, stop} -> From ! "Game Server Stopped! ", void
    end.
...
game_loop(Random_number) ->
    receive
      ...
      {From, Guessed_number}
	  when Guessed_number < Random_number ->
	  From ! "Try a bigger number. ", game_loop(Random_number)
    end.
